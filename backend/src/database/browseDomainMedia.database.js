...
function findById(id) {
  const query = `SELECT * FROM domain_media_browse WHERE id = ?`;
  return db.prepare(query).get(id);
}

function create(domain, imageUrl) {
  const query = `
    INSERT INTO domain_media_browse (domain, image_url)
    VALUES (?, ?)
  `;
...
function countByImageUrl(imageUrl) {
  const query = `SELECT COUNT(*) AS total FROM domain_media_browse WHERE image_url = ?`;
  return Number(db.prepare(query).get(imageUrl)?.total || 0);
}

function remove(id) {
  const query = `DELETE FROM domain_media_browse WHERE id = ?`;
