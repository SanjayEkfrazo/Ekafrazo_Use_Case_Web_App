...
export function updateAccessUser(userId, data) {
  return api.patch(`/access/users/${encodeURIComponent(String(userId))}`, {
    fullName: String(data?.fullName || "").trim(),
    workEmail: String(data?.workEmail || "").trim(),
    organization: String(data?.organization || "").trim(),
    purpose: String(data?.purpose || "").trim(),
...
export function deleteAccessUser(userId) {
  return api.delete(`/access/users/${encodeURIComponent(String(userId))}`);
}
