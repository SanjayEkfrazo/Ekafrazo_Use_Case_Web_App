    ...
    <section className={`rounded-2xl border border-border bg-hero-glow shadow-card ${compact ? "flex min-h-0 flex-col justify-between px-4 py-3.5 md:px-5 md:py-4" : "px-6 py-6 md:px-7 md:py-7"} ${className}`}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="secondary" className={compact ? "px-3 py-1.5 text-xs" : "px-3.5 py-2"} onClick={onSearch}>
            <Search className="h-4 w-4" />
            Search Use Cases
          </Button>
          <Button variant="secondary" className={compact ? "px-3 py-1.5 text-xs" : "px-3.5 py-2"} onClick={onBrowse}>
            Browse Use Cases
            <ArrowRight className="h-4 w-4" />
          </Button>
          {isAdmin && (
            <Button className={compact ? "px-3 py-1.5 text-xs" : "px-3.5 py-2"} onClick={onCreate}>
